...
7. Repeat on a 120/144 Hz display when available. If the current machine is not running at 120 or 144 Hz, record that sub-check as HUMAN_REQUIRED; do not substitute another refresh rate.
...
The original create/update/draw/destroy fragmentation formulation is superseded. The replacement TCW-PROTO-003 mirror gate covers authenticated scene-state transmission, browser-authoritative continuity during disconnect, reconnect resynchronization, matching frame/resize acknowledgements, and both explicit resize directions. TCW-PROTO-003 and TCW-CONT-001 may be marked PASS from redacted human evidence covering those checks; the exact 120/144 Hz sub-check remains HUMAN_REQUIRED when unavailable. Automated Node acknowledgements alone do not satisfy these gates.

...
class CubeRenderer {
  public:
    CubeRenderer(wgpu::Device device, ComPtr<ID3D12Device> nativeDevice, ComPtr<ID3D12CommandQueue> nativeQueue, bool canonical)
        ...
        const float vertices[] = {
            // six faces, four vertices each: position followed by browser-compatible UV
            ...
            -0.5f, 0.5f,-0.5f, 0,1,  0.5f,0.5f,-0.5f, 1,1,  0.5f,0.5f, 0.5f, 1,0, -0.5f,0.5f, 0.5f, 0,0,
            -0.5f,-0.5f, 0.5f, 0,1,  0.5f,-0.5f, 0.5f, 1,1,  0.5f,-0.5f,-0.5f, 1,0, -0.5f,-0.5f,-0.5f, 0,0};
        const uint16_t indices[] = {0,1,2,2,3,0, 4,5,6,6,7,4, 8,9,10,10,11,8, 12,13,14,14,15,12, 16,17,18,18,19,16, 20,21,22,22,23,20};
        ...
        wgpu::TextureDescriptor td{}; td.size={256,256,1}; td.format=wgpu::TextureFormat::RGBA8Unorm; td.usage=wgpu::TextureUsage::TextureBinding|wgpu::TextureUsage::CopyDst; textureInput_=device_.CreateTexture(&td);
        wgpu::SamplerDescriptor sd{}; sd.minFilter=wgpu::FilterMode::Nearest; sd.magFilter=wgpu::FilterMode::Nearest; sampler_=device_.CreateSampler(&sd);
        wgpu::BindGroupEntry entries[3]{}; entries[0].binding=0; entries[0].buffer=uniform_; entries[0].size=256; entries[1].binding=1; entries[1].textureView=textureInput_.CreateView(); entries[2].binding=2; entries[2].sampler=sampler_;
        wgpu::BindGroupDescriptor bgd; bgd.layout=bindLayout_; bgd.entryCount=3; bgd.entries=entries; bindGroup_=device_.CreateBindGroup(&bgd);
    ...
    void UpdateTexture(std::span<const uint8_t> pixels) { wgpu::TexelCopyTextureInfo dst{}; dst.texture=textureInput_; wgpu::TexelCopyBufferLayout layout{}; layout.bytesPerRow=256*4; layout.rowsPerImage=256; wgpu::Extent3D size{256,256,1}; queueWeb_.WriteTexture(&dst,pixels.data(),pixels.size(),&layout,&size); }
  ...
  private:
    ...
    void BuildPipeline(bool canonical) {
        wgpu::BindGroupLayoutEntry be[3]{}; be[0].binding=0; be[0].visibility=wgpu::ShaderStage::Vertex; be[0].buffer.type=wgpu::BufferBindingType::Uniform; be[1].binding=1; be[1].visibility=wgpu::ShaderStage::Fragment; be[1].texture.sampleType=wgpu::TextureSampleType::Float; be[1].texture.viewDimension=wgpu::TextureViewDimension::e2D; be[2].binding=2; be[2].visibility=wgpu::ShaderStage::Fragment; be[2].sampler.type=wgpu::SamplerBindingType::Filtering;
        wgpu::BindGroupLayoutDescriptor bld; bld.entryCount=3; bld.entries=be; bindLayout_=device_.CreateBindGroupLayout(&bld); wgpu::PipelineLayoutDescriptor pld; pld.bindGroupLayoutCount=1; pld.bindGroupLayouts=&bindLayout_; wgpu::PipelineLayout layout=device_.CreatePipelineLayout(&pld);
        const char* source="struct U { mvp: mat4x4f }; @group(0) @binding(0) var<uniform> u:U; @group(0) @binding(1) var tex:texture_2d<f32>; @group(0) @binding(2) var samp:sampler; struct V { @location(0) pos:vec3f, @location(1) uv:vec2f }; struct O { @builtin(position) pos:vec4f, @location(0) uv:vec2f }; @vertex fn vs(v:V)->O { var o:O; o.pos=u.mvp*vec4f(v.pos,1); o.uv=v.uv; return o; } @fragment fn fs(o:O)->@location(0) vec4f { return textureSample(tex,samp,o.uv); }";
        std::string shader(source); (void)canonical;
        wgpu::ShaderSourceWGSL wgsl; wgsl.code = shader.c_str(); wgpu::ShaderModuleDescriptor sd; sd.nextInChain = &wgsl; wgpu::ShaderModule sm = device_.CreateShaderModule(&sd); wgpu::VertexAttribute attrs[2]; attrs[0].format = wgpu::VertexFormat::Float32x3; attrs[0].offset = 0; attrs[0].shaderLocation = 0; attrs[1].format = wgpu::VertexFormat::Float32x2; attrs[1].offset = 12; attrs[1].shaderLocation = 1; wgpu::VertexBufferLayout vb; vb.arrayStride = 20; vb.attributeCount = 2; vb.attributes = attrs; wgpu::ColorTargetState ct; ct.format = wgpu::TextureFormat::RGBA8Unorm; wgpu::FragmentState fs; fs.module = sm; fs.entryPoint = "fs"; fs.targetCount = 1; fs.targets = &ct; wgpu::DepthStencilState ds; ds.format = wgpu::TextureFormat::Depth24Plus; ds.depthWriteEnabled = true; ds.depthCompare = wgpu::CompareFunction::Less; wgpu::RenderPipelineDescriptor rp; rp.layout = layout; rp.vertex.module = sm; rp.vertex.entryPoint = "vs"; rp.vertex.bufferCount = 1; rp.vertex.buffers = &vb; rp.fragment = &fs; rp.depthStencil = &ds; pipeline_ = device_.CreateRenderPipeline(&rp);
    }
    ...
    wgpu::Device device_; wgpu::Queue queueWeb_; ComPtr<ID3D12Device> d3d_; ComPtr<ID3D12CommandQueue> queue_; ComPtr<ID3D12Resource> nativeTexture_; wgpu::SharedTextureMemory memory_; wgpu::Texture texture_, depth_, textureInput_; wgpu::TextureView colorView_, depthView_; wgpu::Sampler sampler_; wgpu::Buffer vertex_, index_, uniform_; wgpu::BindGroupLayout bindLayout_; wgpu::BindGroup bindGroup_; wgpu::RenderPipeline pipeline_; uint32_t width_ = 0, height_ = 0; bool textureInitialized_ = false; Readback readback_; ComPtr<ID3D12CommandAllocator> allocator_ = nullptr; ComPtr<ID3D12GraphicsCommandList> list_; ComPtr<ID3D12Fence> fence_; HANDLE event_ = nullptr; uint64_t serial_ = 0;
};
...
void Renderer::Submit(const SceneState& state,uint64_t dropped,const std::string& capture,std::vector<std::uint8_t>* output) {
    ...
    const std::string title="FrameBridge Native Mirror | native-dawn | reference-upscale | input "+std::to_string(i.inputWidth)+"x"+std::to_string(i.inputHeight)+" -> output "+std::to_string(i.outputWidth)+"x"+std::to_string(i.outputHeight)+" | "+std::to_string(i.renderScale)+"x NOT DLSS | "+i.name+" | received/submitted "+
...
void Renderer::UpdateTexture(std::span<const std::uint8_t> pixels) { if(pixels.size()!=256u*256u*4u) throw std::runtime_error("texture upload size="+std::to_string(pixels.size())); impl_->cube->UpdateTexture(pixels); }

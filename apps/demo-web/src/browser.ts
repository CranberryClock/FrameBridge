...
let socket:WebSocket|undefined,mirror:MirrorClient|undefined;let cameraDistance=3;let received=0,displayed=0;let textureRevision=1n;const imageTimes:number[]=[];const sentAt=new Map<bigint,number>();let lastImageAt=0;
...
function connect(){
 ...
 ws.onmessage=event=>{if(socket!==ws)return;
  try{
   if(typeof event.data==="string"){const caps=client.authenticate(event.data);client.uploadTexture(textureBytes(textureRevision),textureRevision);clearProtocolError();tokenInput.value="";put("generation",caps.sessionGeneration);put("backend",caps.backend);put("authentication","authenticated");put("connection","connected");put("texture-revision",textureRevision+" / awaiting");sendFrame();return;}

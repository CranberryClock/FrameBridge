...
const soakCommit=acceptance.data.implementation_commit;
...
const clean=path.join(root,"out/tcw004-clean-minor/artifacts/tcw-004/validation");
const dirs=await readdir(clean);let validationDir;
for(const candidate of dirs.filter(name=>name.startsWith("run-"))) {
 try { const record=await read(path.join(clean,candidate,"result.json")); if(record.implementation_commit===sha&&record.clean_install){validationDir=candidate;break;} } catch {/* incomplete run */}
}
if(!validationDir)throw new Error("clean validation missing");
...
const human=await read(path.join(base,"human-browser-evidence.json"));
const changed=execFileSync("git",["diff","--name-only","97a5367155d5f580e863561322eab712bd2ddb32",sha],{encoding:"utf8"}).trim().split(/\r?\n/);
const gates={"TCW-BUILD-001":validation.status,"TCW-PROTO-001":validation.status==="PASS"?"PASS":"FAIL","TCW-PROTO-002":validation.status==="PASS"?"PASS":"FAIL","TCW-PROTO-003":human.gate_status["TCW-PROTO-003"].startsWith("PASS")?"PASS":"HUMAN_REQUIRED","TCW-PROTO-004":acceptance.data.status==="PASS"&&acceptance.data.elapsed_seconds>=1800&&diagnostics.every(r=>r.data.status==="PASS")?"PASS":"FAIL","TCW-CONT-001":human.gate_status["TCW-CONT-001"].startsWith("PASS")?"PASS":"HUMAN_REQUIRED"};
const manifest={task:"TCW-004",status:"SUPERVISOR_REVIEW_HUMAN_REQUIRED",implementation_commit:sha,acceptance_source_commit:soakCommit,acceptance_reused_from_prior_commit:soakCommit!==sha,gates,scope_boundary:"Node loopback test/developer infrastructure; browser WebGPU renderer; no native socket or renderer claim",stale_evidence:"Prior real-soak.json invalidated; retained in Git history only",fixture_counts:{valid_binary:12,malformed_binary:11,json:2},test_results:testResults,cpp_output:await readFile(path.join(target,"native-execute.txt"),"utf8"),validation:{path:path.relative(base,target),...validation},diagnostics:diagnostics.map(r=>({path:"runs/"+r.dir+"/result.json",status:r.data.status,process_exit_code:r.data.process_exit_code,elapsed_seconds:r.data.elapsed_seconds,average_attempted_hz:r.data.average_attempted_hz})),acceptance:{path:"runs/"+acceptance.dir+"/result.json",...acceptance.data},human_evidence:"human-browser-evidence.json",ci,changed_files:changed,human_driver_checklist:"human-browser-steps.md",limitations:["The accepted wall-clock soak predates this minor client/UI commit; the soak runner and wire contract were unchanged, and fresh clean validation plus diagnostics cover the revised code.","Memory sampling uses explicit GC; both pre-GC and retained heap are recorded.","No native networking or rendering is implemented in TCW-004."]};

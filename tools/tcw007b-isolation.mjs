...
await wait(7000);ws.close();child.stdin.end('\n');await wait(500);if(!child.killed&&child.exitCode===null)child.kill();const stages=lines.filter(x=>x.startsWith('{')).slice(-20);console.log(JSON.stringify({mode,port:ready.port,acks,images,consumed,client_error:error,stderr,process_exit:child.exitCode,stages}));
